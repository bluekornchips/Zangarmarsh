...
Runs a fixed sequence to sync a development machine with this repo: ensure `jq`, install the local quest-log plugin, and sync `.vscode` settings from Zangarmarsh into the current git root via `questlog`. Optional cleanup runs only with `--force`.
...
3. `questlog` — install the quest-log plugin under `~/.cursor/plugins/local/quest-log`, and sync `.vscode/` via `tools/quest-log/quest-log.sh`
